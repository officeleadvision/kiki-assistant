    ...
    try:
        ...
        if ENABLE_FORWARD_USER_INFO_HEADERS and user:
            ...
            if metadata and metadata.get("chat_id"):
                headers["X-Lead Me-Chat-Id"] = metadata.get("chat_id")
        ...
        if r.ok is False:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f"Lead Me: Server Connection Error",
                )
        ...
        raise HTTPException(
            ...
            detail=detail if e else "Lead Me: Server Connection Error",
        )
    finally:
        if not stream:
            await cleanup_response(r, session)
...
async def verify_connection(
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Lead Me: Server Connection Error"
            )
        except Exception as e:
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = "Lead Me: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
