		...
		>
			...
			{#if !$temporaryChatEnabled && chat?.id}
				...
				{#if $folders.length > 0}
					<DropdownMenu.Sub>
						<DropdownMenu.SubTrigger
							class="flex gap-2 items-center px-3 py-1.5 text-sm cursor-pointer hover:bg-primary/5 dark:hover:bg-gray-800 rounded-xl select-none w-full"
						>
						...
						<DropdownMenu.SubContent
							class="w-full rounded-2xl p-1 z-50 bg-white text-primary border border-gray-100 shadow-lg max-h-52 overflow-y-auto scrollbar-hidden dark:bg-gray-850 dark:text-white dark:border-gray-800"
							transition={flyAndScale}
							sideOffset={8}
						>
