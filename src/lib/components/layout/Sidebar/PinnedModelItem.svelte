...
{#if model}
	...
	<div
		class=" flex justify-center text-primary cursor-grab relative group"
		data-id={model?.id}
		on:mouseenter={(e) => {
			mouseOver = true;
		}}
	...
	>
		<a
			class="grow flex items-center space-x-2.5 rounded-xl px-2.5 py-[7px] group-hover:bg-primary/5 transition"
			href="/?model={model?.id}"
			on:click={onClick}
			draggable="false"
		>
			...
			<div class="absolute right-5 top-2.5">
				<div class=" flex items-center self-center space-x-1.5">
					<Tooltip content={$i18n.t('Unpin')} className="flex items-center">
						<button
							class=" self-center hover:text-primary transition"
							on:click={() => {
								onUnpin();
							}}
							type="button"
